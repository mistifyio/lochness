...
systemd service (locker service) which is charged with starting the user supplied
program, as a systemd service. Such nested, so systemd. I know.
...
to do work systemd will kill it. Meanwhile, the user service is
configured with `BindsTo=locker.service` so that if the locker service dies
(via watchdog or other) the user service is killed. And since all the
