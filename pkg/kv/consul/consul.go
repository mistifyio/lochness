...
func (c *ckv) Lock(key string, ttl time.Duration) (kv.Lock, error) {
	...
	l := &lock{session: c.client.Session(), name: session, kv: c.c, key: key}
	if err != nil {
		return nil, err
	}
	return l, nil
...
func (c *ckv) EphemeralKey(key string, ttl time.Duration) (kv.EphemeralKey, error) {
	...
	l := &ekey{kv: c, lock: lock{session: c.client.Session(), name: session}}
	return l, err
}

func (l *lock) Renew() error {
	entry, _, err := l.session.Renew(l.name, nil)
	if err != nil {
		return err
	}
	if entry == nil {
...
func (l *lock) unlock() error {
	...
	ok, _, err := l.kv.Release(&consul.KVPair{Key: l.key, Session: l.name}, nil)
	if err != nil {
		return err
	}
	if !ok {
